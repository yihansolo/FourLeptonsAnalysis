...
// Tue Oct  9 11:20:25 2018 by ROOT version 6.13/02
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   static constexpr Int_t kMaxParticle = 31;
...
#ifdef LHEF4_cxx
LHEF4::LHEF4(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("tag_1_pythia_lhe_events.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("tag_1_pythia_lhe_events.root");
      }
...
Int_t LHEF4::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t LHEF4::LoadTree(Long64_t entry)
{
...
void LHEF4::Show(Long64_t entry)
{
...
Int_t LHEF4::Cut(Long64_t entry)
{
